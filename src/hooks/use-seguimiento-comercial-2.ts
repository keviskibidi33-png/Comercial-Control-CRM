...
export function useSeguimientoComercial2(filters: { search?: string; estado_cliente?: string; limit?: number; offset?: number } = {}) {
  const queryClient = useQueryClient()
  const queryKey = ["seguimiento-comercial-2", filters]
  ...
  const { data, error: dataError, isLoading, refetch } = useQuery<{ total: number; items: SeguimientoRow[] }>({
    ...
    queryFn: () => {
      ...
      return fetchWithAuth(`/api/seguimiento-comercial-2?${params.toString()}`)
    },
    staleTime: 15000,
    retry: false,
  ...
  const { data: catalogs, error: catalogsError } = useQuery<Catalogs>({
    queryKey: ["seguimiento-comercial-2-catalogs"],
    queryFn: () => fetchWithAuth("/api/seguimiento-comercial-2/catalogs"),
    staleTime: 60000,
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: Partial<SeguimientoRow> }) => {
      return fetchWithAuth(`/api/seguimiento-comercial-2/${id}`, {
        method: "PATCH",
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["seguimiento-comercial-2-catalogs"] })
    },
  })
  ...
  const insertMutation = useMutation({
    mutationFn: async (newRow: Partial<SeguimientoRow>) => {
      return fetchWithAuth("/api/seguimiento-comercial-2", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["seguimiento-comercial-2"] })
      queryClient.invalidateQueries({ queryKey: ["seguimiento-comercial-2-catalogs"] })
    },
    onError: (err) => {
  ...
  const importMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      return fetchWithAuth("/api/seguimiento-comercial-2/import", {
        method: "POST",
        body: formData,
      })
    },
  ...
  const exportToExcel = async (options?: { asesor?: string; ids?: number[]; filename?: string }) => {
    try {
      ...
      for (let i = 0; i < baseUrls.length; i += 1) {
        const res = await fetch(`${baseUrls[i]}/api/seguimiento-comercial-2/export${queryStr}`, { headers })
        if (res.ok) {
          response = res
          break
        }
